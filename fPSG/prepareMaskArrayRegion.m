function [nucleiMasksROI, num] = prepareMaskArrayRegion(L4, dirname, dirname2, filenames, ch_max, cropHeight, cropWidth)
%
...
%
%Syntax:    [nucleiMasksROIs, num] = prepareMaskArrayRegion()
...
J = imread(fullfile(dirname, 'motionCorrected', filenames{1}));
J_cropped = J(1:cropHeight, 1:cropWidth);

%import GCaMP last timepoint and crop
JLast = imread(fullfile(dirname, 'motionCorrected', filenames{size(filenames,2)-1}));
...
figure; imshow(J_sum);
export_fig(J_sum, [dirname,filesep,'Analysis',filesep,'nuclei_ColorROI_overlay.tif'], '-r300', '-a3');
print('-depsc','-r300',[dirname,filesep,'Analysis',filesep,'nuclei_ColorROI_overlay.eps']);
close(gcf);
...
export_fig(overlay2, [dirname,filesep,'Analysis',filesep,'first_frame_with_mask.tif'], '-r300', '-a3');
close(gcf);

%check if ROIs shifted significantly during timelapse
figure; imshow(overlay3);
export_fig(overlay3, [dirname,filesep,'Analysis',filesep,'last_frame_with_mask.tif'], '-r300', '-a3');
close(gcf);
end