...
%G edit here to allow the user to set the position of the menu with
%'position'
for i = 1:size(varargin,2)
    if strcmp(varargin{1,i},'position')
        pos = varargin{1,i+1};
        varargin = [varargin(1:i-1) varargin(i+2:size(varargin,2))];
        break;
    else
        pos = [];
    end
...
if useGUI,
    ...
    k = local_GUImenu( xHeader, ArgsIn, pos );
else,
...
%%#########################################################################
function k = local_GUImenu( xHeader, xcItems, pos )
...
%G edit for moving the menu around the screen
if ~isempty(pos)
    set(menuFig,'Units','pixels');
    x = get(menuFig,'position');
    if ischar(pos)
        movegui(menuFig,pos);        
    else
        set(menuFig,'Position',[pos x(1,3:4)]);
    end
end
